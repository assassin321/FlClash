...
ご利用のプラットフォームおよびシステムに適した最新のインストールパッケージを **[[Releases]](https://github.com/assassin321/FlClash/releases)** ページからダウンロードしてください。
...
**その他のインストール方法:**<br>
**ArchLinux:** <code>yay -S flclash-bin または paru -S flclash-bin</code> ([lyj404](https://github.com/lyj404/flclash-aur) により維持)<br>
**AMD64=v1:** <code>yay -S flclash-compatible-bin または paru -S flclash-compatible-bin</code> ([VillagerTom](https://github.com/VillagerTom) により維持)
...
1. **インストール・起動およびセキュリティ**：
   ...
   - **セキュリティ：FlClash は完全オープンソースかつ透明で、コードは Signpath のセキュリティ監査に合格しています。**
...
3. **macOSインストールの注意事項**：
   - ご利用の環境（Intel / Apple Silicon）に適したファイルをダウンロードし、`FlClash-macos-xx.dmg` をダブルクリックして開きます。
   - FlClash アイコンを「Applications」フォルダにドラッグ＆ドロップしてインストール完了。
   - **インストールまたはアップデート時の Gatekeeper 回避**（[現在 Apple デベロッパー証明書を購入していないため](https://support.apple.com/en-us/102445)）：
     - **推奨**：「Applications」フォルダ内で **FlClash アイコンを右クリック**し、**「開く」** を選択後、確認ダイアログで再度 **「開く」** をクリックします。
     - **代替案**：ダブルクリックで開けない場合は、Mac「システム設定」 -> 「プライバシーとセキュリティ」から FlClash を探して **「このまま開く」** をクリックします。
...
* v1.18.8 より、FlClash は外部オーバーライドスクリプトの UI 適応をサポートしています。例えば AIsouler の **[スクリプト/設定リポジトリ](https://github.com/AIsouler/MyClash)** の場合、スクリプトの1行目に以下を追加するだけで、FlClash 内蔵のトグルスイッチを直接使用できます：
...
すべての コントリビューター および利用・参考にしたオープンソースプロジェクトに感謝いたします：
