...
class AppController {
  ...
  Future<void> updateTraffic() async {
    ...
    if (networkSpeedNotification && isScreenOn) {
      ...
      final profile = profiles
          ...
          .firstOrNull;
      final profileName = profile?.label ?? 'FlClash';
    ...
    if (data != null) {
      ...
      if (assetSuffix.isNotEmpty) {
        final versionWithoutV = tagName.startsWith('v')
            ...
            : tagName;
        downloadUrl =
            'https://github.com/$repository/releases/download/$tagName/FlClash-$versionWithoutV-$assetSuffix';
      }
  ...
  Future<List<int>> backupData() async {
    ...
    return Isolate.run<List<int>>(() async {
      ...
      final tempZipPath = join(
        tempDir.path,
        'flclash_backup_${DateTime.now().millisecondsSinceEpoch}.zip',
      );
      ...
      final tempMarkerFile = File(
        join(
          tempDir.path,
          'flclash_marker_${DateTime.now().millisecondsSinceEpoch}.tmp',
        ),
      );
      await tempMarkerFile.writeAsBytes(markerBytes);
      await encoder.addFile(tempMarkerFile, '.flclash_marker');
      ...
      final tempConfigFile = File(
        join(
          tempDir.path,
          'flclash_config_${DateTime.now().millisecondsSinceEpoch}.tmp',
        ),
      );
  ...
  Future<void> _recoveryFromArchive(
    ...
    // Check for FlClash marker
    final hasFlClashMarker = archive.files.any(
      (file) => file.name == '.flclash_marker',
    );

    if (hasFlClashMarker) {
      // FlClash backup
      await _recoveryFlClashBackup(archive, recoveryOption, homeDirPath);
    } else {
      // Legacy backup
  ...
  /// Restore FlClash
  Future<void> _recoveryFlClashBackup(
    Archive archive,
    RecoveryOption recoveryOption,
    String homeDirPath,
  ) async {
    ...
    final configs = archive.files
        .where(
          (item) =>
              item.name.endsWith('.json') && item.name != '.flclash_marker',
        )
        .toList();
    final profiles = archive.files.where(
      (item) => !item.name.endsWith('.json') && item.name != '.flclash_marker',
    );
