...
import 'package:fl_clash/common/helper_auth.dart';
    ...
    if (system.isMacOS) {
      ...
      if (xattrCheck.exitCode == 0) {
        ...
        if (removeResult.exitCode == 0) {
          commonPrint.log('Cleared quarantine attribute from FlClashCore');
        } else {
      ...
      if (result.exitCode != 0) {
        if (!quarantineCleared) {
          globalState.showNotifier(
            'Failed to authorize FlClashCore. Try: xattr -dr com.apple.quarantine /Applications/FlClash.app',
          );
        } else {
...
class MacOS {
  ...
  Future<void> _setDns() async {
    ...
    // overwrite the real system DNS backup with FlClash's own hijack DNS.
