package com.assassin321.flclash.plugins
...
import com.assassin321.flclash.FlClashApplication
import com.assassin321.flclash.GlobalState
import com.assassin321.flclash.R
import com.assassin321.flclash.extensions.awaitResult
import com.assassin321.flclash.extensions.getActionIntent
import com.assassin321.flclash.models.Package
...
class AppPlugin : FlutterPlugin, MethodChannel.MethodCallHandler, ActivityAware {
    ...
    private val iconCacheDir by lazy {
        File(FlClashApplication.getAppContext().cacheDir, "app_icons").apply {
            if (!exists()) mkdirs()
        }
    }
    ...
    private fun initShortcuts(labels: Map<*, *>) {
        val toggleShortcut = ShortcutInfoCompat.Builder(FlClashApplication.getAppContext(), "toggle")
            .setShortLabel(labels["toggle"]?.toString() ?: "Toggle")
            .setIcon(createBitmapIcon(FlClashApplication.getAppContext(), R.drawable.ic_shortcut_toggle))
            .setIntent(FlClashApplication.getAppContext().getActionIntent("CHANGE"))
            .build()

        val startShortcut = ShortcutInfoCompat.Builder(FlClashApplication.getAppContext(), "start")
            .setShortLabel(labels["start"]?.toString() ?: "Start")
            .setIcon(createBitmapIcon(FlClashApplication.getAppContext(), R.drawable.ic_shortcut_start))
            .setIntent(FlClashApplication.getAppContext().getActionIntent("START"))
            .build()

        val stopShortcut = ShortcutInfoCompat.Builder(FlClashApplication.getAppContext(), "stop")
            .setShortLabel(labels["stop"]?.toString() ?: "Stop")
            .setIcon(createBitmapIcon(FlClashApplication.getAppContext(), R.drawable.ic_shortcut_stop))
            .setIntent(FlClashApplication.getAppContext().getActionIntent("STOP"))
            .build()
    ...
    private fun isSystemInDarkMode(): Boolean {
        val nightModeFlags = FlClashApplication.getAppContext().resources.configuration.uiMode and
            Configuration.UI_MODE_NIGHT_MASK
        return nightModeFlags == Configuration.UI_MODE_NIGHT_YES
    }

    private fun isAndroidTV(): Boolean {
        val uiMode = FlClashApplication.getAppContext().resources.configuration.uiMode
    ...
    private fun updateExcludeFromRecents(value: Boolean?) {
        val am = FlClashApplication.getAppContext().getSystemService(Context.ACTIVITY_SERVICE) as? ActivityManager
    ...
    private fun getIconSizePx(): Int {
        val density = FlClashApplication.getAppContext().resources.displayMetrics.density
    ...
    private fun getDefaultIconBytes(): ByteArray? = runCatching {
        drawableToPngBytes(FlClashApplication.getAppContext().packageManager.defaultActivityIcon, getIconSizePx())
    }.getOrNull()
    ...
    private suspend fun getPackageIconBytes(packageName: String, forceRefresh: Boolean = false): ByteArray? =
        withContext(Dispatchers.IO) {
            val pm = FlClashApplication.getAppContext().packageManager ?: return@withContext null
            runCatching {
    ...
    private suspend fun getPackages(forceRefresh: Boolean = false): List<Package> = withContext(Dispatchers.IO) {
        ...
        val pm = FlClashApplication.getAppContext().packageManager ?: return@withContext emptyList()
        val selfPackageName = FlClashApplication.getAppContext().packageName
    ...
    fun requestVpnPermission(callBack: () -> Unit) {
        ...
        val intent = runCatching { VpnService.prepare(FlClashApplication.getAppContext()) }.getOrNull()
        if (intent != null) {
    ...
    fun requestNotificationsPermission() {
        ...
        if (ContextCompat.checkSelfPermission(FlClashApplication.getAppContext(), Manifest.permission.POST_NOTIFICATIONS) == PackageManager.PERMISSION_GRANTED) return
        activityRef?.get()?.let {
    ...
    private fun isIgnoringBatteryOptimizations(): Boolean =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            val powerManager = FlClashApplication.getAppContext().getSystemService(android.content.Context.POWER_SERVICE) as? android.os.PowerManager
            powerManager?.isIgnoringBatteryOptimizations(FlClashApplication.getAppContext().packageName) ?: false
        } else true

    private fun requestIgnoreBatteryOptimizations() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) return
        val intent = Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS).apply {
            data = Uri.parse("package:${FlClashApplication.getAppContext().packageName}")
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        }
        runCatching { FlClashApplication.getAppContext().startActivity(intent) }
            .onFailure {
                runCatching {
                    FlClashApplication.getAppContext().startActivity(Intent(Settings.ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS).apply {
                        addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                    })
                }
            }
    ...
    private fun setLauncherIcon(useDarkIcon: Boolean) {
        ...
        val defaultComponent = android.content.ComponentName(packageName, "com.assassin321.flclash.MainActivity")
        val lightComponent = android.content.ComponentName(packageName, "com.assassin321.flclash.MainActivityLight")
        val darkComponent = android.content.ComponentName(packageName, "com.assassin321.flclash.MainActivityDark")
        ...
        com.assassin321.flclash.services.NotificationComponentCache.invalidate()
    ...
    private fun hasCameraPermission(): Boolean =
        ContextCompat.checkSelfPermission(FlClashApplication.getAppContext(), Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED
    ...
    private fun openAppSettings() {
        ...
        activityRef?.get()?.startActivity(intent) ?: run {
            ...
            FlClashApplication.getAppContext().startActivity(intent)
        }
    }
