package com.assassin321.flclash.services
...
suspend fun Service.createFlClashNotificationBuilder(
    isSuspended: Boolean = GlobalState.isSmartStopped,
    isHighPriority: Boolean = GlobalState.isNotificationHighPriority
): NotificationCompat.Builder =
    withContext(Dispatchers.IO) {
        val defaultComponent = ComponentName(packageName, "com.assassin321.flclash.MainActivity")
        val lightComponent = ComponentName(packageName, "com.assassin321.flclash.MainActivityLight")
        val darkComponent = ComponentName(packageName, "com.assassin321.flclash.MainActivityDark")
        ...
        val pendingIntent = withContext(Dispatchers.Main) {
            PendingIntent.getActivity(this@createFlClashNotificationBuilder, 0, intent, flags)
        }
        ...
        NotificationCompat.Builder(this@createFlClashNotificationBuilder, channelId).apply {
...
fun Service.ensureNotificationChannel(
    ...
    if (channel == null) {
        ...
        val name = when {
            isSuspended -> "FlClash Suspended Service"
            isHighPriority -> "FlClash High Priority Service"
            else -> "FlClash Service"
        }
